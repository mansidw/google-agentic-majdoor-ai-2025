...
def create_insight_pass():
    ...
    for p in existing_passes:
        for tm in p.get("textModulesData", []):
            if (
                ...
                and tm.get("body", "").strip() == description.strip()
            ):
    ...
    object_suffix = f"{insight_type}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}"
    text_modules = [
        {
    ...
    return jsonify({"object_suffix": object_suffix, "class_suffix": class_suffix, "object_data": object_data, "reused": False})
